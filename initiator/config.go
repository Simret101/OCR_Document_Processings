...
func InitConfig(config Config) error {
	...
	for i, name := range config.Names {
		...
		config.Logger.Info("Config loaded successfully",
			zap.String("config", name+"."+config.Type))
	}
	...
	if err := setupConfigWatcher(config.Logger); err != nil {
		config.Logger.Warn("Failed to setup config watcher", zap.Error(err))
	}

	config.Logger.Info("All configurations initialized successfully",
...
func setupConfigWatcher(log *zap.Logger) error {
	...
	viper.OnConfigChange(func(e fsnotify.Event) {
		...
		if err := validateConfig(); err != nil {
			log.Error("Invalid configuration after change", zap.Error(err))
		}
	})
	return nil
}
